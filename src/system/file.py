...
def openFile(extension=None, defaultLocation=None):
    """
    Shows an "Open File" dialog box, prompting the user to choose a file
    to open. Returns the path to the file that the user chose, or None
    if the user canceled the dialog box. An extension can optionally be
    passed in that sets the filetype filter to that extension.
...
def openFiles(extension=None, defaultLocation=None):
    """
    Shows an "Open File" dialog box, prompting the user to choose a file
    or files to open. Returns the paths to the files that the user
...
def writeFile(filepath, data, append=False):
    ...
    exists, the append argument determines whether or not it is
