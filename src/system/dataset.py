...
__all__ = [
    ...
    'clearDataset', 'dataSetToExcel', 'dataSetToHTML', 'deleteRow',
    'deleteRows', 'exportCSV', 'exportExcel', 'exportHTML', 'filterColumns',
    'formatDates', 'fromCSV', 'getColumnHeaders', 'setValue', 'sort', 'toCSV',
    'toDataSet', 'toPyDataSet', 'updateRow'
]
...
def addColumn(dataset, colIndex, col, colName, colType):
    """Takes a dataset and returns a new dataset with a new column
    added or inserted into it. Datasets are immutable, so it is
...
def exportHTML(filename, showHeaders, dataset, title):
    """Exports the contents of a dataset to an HTML page. Prompts the
    user to save the file to disk.
