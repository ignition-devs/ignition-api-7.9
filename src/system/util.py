...
def exit(force=False):
    ...
    Args:
        force (bool): f True (1), the shutdown-intercept script will be
...
def getClientId():
    ...
    running client's session. You are guaranteed that this number is
    unique between all running clients.
...
def getInactivitySeconds():
    ...
    activity. Note - this function will always return zero in the
    Designer.
...
def retarget(
    ...
    Args:
        projectName (str): The name of the project to retarget to.
        gatewayAddress (str): The address of the Gateway that the
            project resides on. If omitted, the current Gateway will be
            used. Format is: "host:httpPort:sslPort/main". Optional.
        ...
        startupWindows (list[str]): A list of window paths to use as the
...
def sendMessage(
    ...
    within a project. These message handlers run Jython code when a
    message is received. You can add message handlers under the
