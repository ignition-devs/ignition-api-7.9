...
def acknowledge(alarmIds, notes=None, username=None):
    ...
    events for the same source. The alarms will be acknowledged by the
    logged in user making the call. Additionally, acknowledgement
    notes may be included and will be stored along with the
    acknowledgement.
...
def createRoster(name, description):
    ...
    Args:
        ...
        description (str): An description for the new roster.
...
def getRosters():
    ...
    Returns:
         dict: A dictionary that maps roster names to a List of
            usernames in the roster. The List of usernames may be
            empty if no users have been added to the roster.
    """
    return dict()
...
def listPipelines():
    """Will return a list of the available Alarm Notification
    Pipelines.

    Returns:
        list[str]: A list of pipeline names. The list may be empty if
            no pipelines exist. Unsaved name changes will not be
...
def queryJournal(startDate=system.date.addHours(system.date.now(), -8),
    ...
    function that can be used to convert the query results into a
    normal dataset, with the columns: EventId, Source, DisplayPath,
    ...
    Args:
        ...
        path (list[str]): A list of possible source paths to search
        ...
        all_properties (list[object]): A set of property conditions,
            all of which must be met for the condition to pass. This
            parameter is a list of tuples, in the form ("propName",
            "condition", value). Valid condition values: "=", "!=",
            "<", "<=", ">", ">=". Only the first two conditions may be
            used for string values.
        any_properties (list[object]): A set of property conditions,
            any of which will cause the overall the condition to pass.
            This parameter is a list of tuples, in the form
            ("propName", "condition", value). Valid condition values:
            "=", "!=", "<", "<=", ">", ">=". Only the first two
    ...
    Returns:
        AlarmQueryResults: The AlarmQueryResults object is
            ...
            AlarmQueryResults object has a built-in getDataset()
            function that will return a Standard Dataset containing
            the Event Id (UUID of the alarm), Source Path, Display
            Path, Event Time, State (as an integer), and Priority (as
            an integer).
    """
...
def queryStatus(priority, state, path, source, displaypath, all_properties,
    ...
    Args:
        ...
        state (list[str]): A list of states to allow. Valid values:
            ...
            "ActiveAcked".
        path (list[str]): A list of possible source paths to search
            at. The wildcard "*" may be used. Works the same as the
            source argument, and either can be used.
        source (list[str]): A list of possible source paths to search
            at. The wildcard "*" may be used. Works the same as the
            path argument, and either can be used.
        displaypath (list[str]): A list of display paths to search at.
            Display paths are separated by "/", and if a path ends in
            "/*", everything below that path will be searched as well.
        all_properties (list[object]): A set of property conditions,
...
def shelve(path, timeoutSeconds, timeoutMinutes):
    ...
    seconds (timeoutSeconds). If an alarm is already shelved, this
    will overwrite the remaining time. To unshelve alarms, this
    function may be used with a time of "0".

    Args:
        path (list[str]): A list of possible source paths to search
            at. If a path ends in "/*", the results will include
            anything below that path.
        timeoutSeconds (int): The amount of time to shelve the
            matching alarms for, specified in seconds. 0 indicates
            that matching alarm events should now be allowed to pass.
        timeoutMinutes (int): The amount of time to shelve the
            matching alarms for, specified in minutes. 0 indicates
            that matching alarm events should now be allowed to pass.
    """
