...
def addTag(parentPath, name, tagType, dataType, accessRights, enabled,
           value, attributes, parameters, overrides, alarmList, alarmConfig):
    """Adds a new tag in Ignition. You can add OPC, memory,
    expression, query, folder, and UDT instance tags. You can't add
    Client Tags, because those can vary from project to project.

    Args:
        parentPath (str): The folder to add the tag to. Leave blank
            for the root folder.
        ...
        dataType (str): The data type of the tag. Not used for UDT
            instances or folders. Possible basic values are Int1,
            Int2, Int4, Int8, Float4, Float8, Boolean, String,
            DataSet, and DateTime. Possible array values are
            Int4Array,  Int8Array, Float8Array, BooleanArray,
        ...
        alarmList (str): List of legacy alarms for the tag. The
            legacy alarm system was retired in 7.6.0, so the
            alarmConfig parameter should be utilized on newer
            versions.
...
def browseConfiguration(path, recursive):
    ...
    information for each Tag within the specified path. This can be
    used to  view event scripts, alarms, as well as any other
    configurable attribute on a Tag.

    Args:
        path (str): The path that will be browsed, typically to a
            folder or UDT instance. Leave blank to browse the root
            folder. A Tag Provider may be specified as follows:
            "[TagProvider]". If the Tag Provider is omitted, client
            scoped calls will be made against the project's default
            provider. Gateway scoped calls must include a Tag
...
def browseHistoricalTags(path, nameFilters, maxSize, continuationPoint):
    ...
    Returns:
        BrowseResults: An object that contains the results as
            well as the Continuation Point. Get the results by calling
...
def editAlarmConfig(tagPaths, alarmConfig):
    ...
    Args:
        tagPaths (list[str]): The full path to the tag you want to
            edit. Note: you can specify the tag provider name in
        ...
        alarmConfig (dict): A dictionary of multi-dimensional lists
            ...
            dictionary will be the name of the alarm being to edit,
            and the value is a list of lists. The nested lists use the
...
def editTags(tagPaths, attributes, parameters, accessRights, overrides,
    ...
    Args:
        tagPaths (list[str]): The full path to the Tag you want to
            edit. For members of UDT instances, the tagPath will be
            the path to the UDT instance, with the overrides parameter
            listing out the member Tags to edit. Note: you can specify
            the Tag provider name in square brackets at the beginning
            of the parentPath string.
            Example: "[myTagProvider]MyTagsFolder". If the Tag
            provider name is left off then the project default
            provider will be used.
        attributes (dict): The Tag's configuration attributes.
        ...
        provider (str): The name of the Tag provider, used in
            conjunction with the JSON argument. The default value is
            the default Tag provider. Optional.
        json (str): The properties to edit on Tags, represented as a
            JSON object. When using this, it acts as a replacement for
            other parameters. Optional.
    """
    print(tagPaths, attributes, parameters, accessRights, overrides,
          alarmList, alarmConfig, provider, json)
...
def queryTagDensity(paths, startDate, endDate):
    """Queries the Tag history system for information about the
    density of data. In other words, how much data is available for a
    given time span.
...
def removeTag(tagPath):
    ...
    Args:
        tagPath (str): The path to the tag you want to remove. You can
            ...
            "[myTagProvider]MyTagsFolder" . If the tag provider name
            is left off then the project default provider will be
            used. Note that an empty path ( '' ) will remove all tags.
    """
    print tagPath
...
def scan(provider, scname):
    """Forces execution of a scan class. On a leased scan class, if
    both the fast and slow rate are set to 0, this will not execute.
...
def storeTagHistory(historyprovider, tagprovider, paths, values,
    ...
    provider, but they do not necessarily need to exist in the
    realtime provider. This means records from non-existent (virtual)
    Tags can be stored in the Tag History system. Because of this, it
    is imperative that Tag paths passed to the function are typed
    precisely, otherwise the history will be stored at an incorrect
    path.

    Note that the Tag History system does cache tag data. Thus, if
    this function is called, the tag path and tag id are cached until
    the history provider or gateway are restarted. This means manually
    ...
    execution table (especially so when working purely with virtual
    tag paths). Instead, the cache must first be cleared, and then a
    new entry will be added the next time this function is called.

    Args:
        historyprovider (str): The historical provider to store to.
        tagprovider (str): The name of the realtime tag provider to
            associate these tags with. The tag provider does not need
            to exist, and the tag paths do not need to exist in it.
        paths (list[str]): A list of paths to store. The values,
            qualities, and timestamps are one-to-one with the paths.
            A single path may be present multiple times in order to
...
def writeAllSynchronous(tagPaths, values, timeout=45000):
    ...
    provided tags. The first write to fail or time out will throw an
    exception, but any subsequent tags in the provided list will still
    be attempted. This function cannot be called from the event
    dispatch thread, meaning it cannot be called directly from a GUI
    event like a button press without creating a new thread with a
...
def writeSynchronous(tagPath, value, timeout=45000):
    """Performs a write to a tag, synchronously. This means that you
    know at the end of this function whether or not the write
    succeeded or not. A write that fails or times out will throw an
    error. However, this function cannot be called from the event
    dispatch thread, which means that it cannot be called directly
    from a GUI event like a button press, without wrapping it in a
    system.util.invokeAsynchronous. You can call this from project
    event scripts like timer scripts.
